...
def init(vr, start_frame, threshold, nflies, file_name, num_bg_frames=50):
    # estimate background
    bg = BackGround(vr)
        ...
        while(ret and res.frame_count < vr.number_of_frames + 1001):
            try:
                ...
                if not ret:
                    ...
                    # display and video save stuff
                    if display is not None and res.frame_count%display==0:
                        frame_with_tracks = fg.annotate(cv2.cvtColor(np.uint8(foreground), cv2.COLOR_GRAY2RGB), centers=np.uint(centers), lines=np.uint(lines))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--file_name', type=str, default="test/160125_1811_1.avi", help='video file to process')
    parser.add_argument('--nflies', type=int, default=1, help='number of flies in video')
    parser.add_argument('-d', '--display', type=int, default=5, help='show every Nth frame')
